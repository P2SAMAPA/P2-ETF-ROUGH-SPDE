...
st.set_page_config(page_title="Rough SPDE Engine", layout="wide")
st.markdown("""
<style>
...
st.markdown('<div class="main-header">🌀 Rough SPDE Engine</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Fractional SPDE | Neural Operator | Rough volatility | Multi‑window evaluation</div>', unsafe_allow_html=True)

st.sidebar.markdown("## 🌀 Rough SPDE")
st.sidebar.markdown(f"**Run Date:** `{st.session_state.get('run_date', 'Not loaded')}`")
st.sidebar.markdown(f"**Next Trading Day:** `{next_trading_day()}`")
st.sidebar.markdown(f"**Hurst:** {config.HURST} | **Time steps:** {config.TIME_STEPS}")
st.sidebar.markdown("**Windows evaluated:** 63, 252, 504, 1008, 2016 days")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rough_spde_' in f]
    if not json_files:
        return None
...
st.header("🏆 Top ETFs by Rough SPDE Predicted Return")
    ...
    for idx, etf in enumerate(top_etfs):
        with cols[idx]:
            ...
            <div class="etf-card">
                ...
                <div class="etf-score">score = {etf['score']:.6f}</div>
...
st.caption("Rough SPDE: du = (L u) dt + dW^H(t,x) where L is a neural operator, and W^H is fractional Brownian motion (Hurst=0.3). The model is trained on spatial fields interpolated from ETF returns. Higher predicted return → stronger long signal.")
